            ...
            if ($_POST['update'] == 'update') {
                ...
                for ($i = 0; $i < count($editedRecipe); $i++) {
                    ...
                    if ($fileInfos[$i]['upFile']['name'] != '') {
                        $imgFileObj = new ImgFile('画像ファイル処理', 8);
                        $fileResult = $imgFileObj->checkUplodeFile($recipeIds[$i], $fileInfos[$i], 0);
                        if (checkClass($fileResult)) {
                            $editResultArr = $fileResult;
                        } else {
                            // 旧画像ファイルの名前を新画像ファイルにつける
                            $editedInfo[$i] = $editedRecipe[$i];
                            $editedInfo[$i]['img'] = $fileResult;
                        }
                    } else {
